...
raw_tri=[line.strip() for line in open('problem067_triangle.txt')]
#format data
raw_tri=raw_tri.split('\n')
triangle=[]
for each in raw_tri:
	triangle.append(map(int, each.split(" ")))

for countdown in range(1,len(triangle[-1])):
	bottom = triangle.pop()
