...
data=data.split('\n')
#I don't know why it keeps picking up an empty index but if I figure it out then this pop function may become unnecessary
data.pop(0)
q=[]
for each in data:
	q.append(map(int, each.split(" ")))
...
for i in range(grid_size-3):
	for j in range(grid_size):
		product = q[i][j]*q[i+1][j]*q[i+2][j]*q[i+3][j]
		if product > maximum_product:
			maximum_product = product
#horizontal products
for i in range(grid_size):
	for j in range(grid_size-3):
		product = q[i][j]*q[i][j+1]*q[i][j+2]*q[i][j+3]
		if product > maximum_product:
			maximum_product = product
#diagonal to lower right
for i in range(grid_size-3):
	for j in range(grid_size-3):
		product = q[i][j]*q[i+1][j+1]*q[i+2][j+2]*q[i+3][j+3]
		if product > maximum_product:
			maximum_product = product
